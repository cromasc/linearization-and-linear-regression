...
y = [
    (
        lambda x, y: [
            round(np.mean(x), 5),
            round(np.sqrt(((np.std(x, ddof=1)) / np.sqrt(len(x))) ** 2 + y ** 2), 5),
        ]
    )(y_raw[i], erro_instrumental)
    for i in range(len(y_raw))
]

x = [
    float(input(f"\nx{i+1}: ")) if i == 0 else float(input(f"x{i+1}: "))
    for i in range(n_dados)
]
...
for i in range(len(erroy)):
    print(f"w{i+1} = {round((1/erroy[i]**2),5)}")
...
print(f"y = {round((a),5)}x - {round(abs(b),5)}\n") if b <= 0 else print(
    f"y = {round((a),5)}x + {round(abs(b),5)}\n"
)

print(
    f"Esses valores consideram o eixo y como sendo o eixo que possui erros estatisticos e/ou experimentais. O eixo x não possui erros. Para calcular os erros no eixo x, basta fazer a inversa da função obtida na regressão, ou seja, \ny = {round((1/a),5)}x - {round(abs(b/a),5)}"
) if b / a >= 0 else print(
    f"Esses valores consideram o eixo y como sendo o eixo que possui erros estatisticos e/ou experimentais. O eixo x não possui erros. Para calcular os erros no eixo x, basta fazer a inversa da função obtida na regressão, ou seja, \ny = {round((1/a),5)}x + {round(abs(b/a),5)}"
)
...
k = (np.e)**(-b)
errok = abs(-(np.e**b)*errob)

print(f'k = {round(k,5)} +/- {round(errok,5)}')
